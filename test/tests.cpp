...
TEST_CASE( "Test Mask.", "[mask], [kernel]" ) {
	for (size_t test = 0; test < 10; test++) {
		...
		gpu.Mask(d_out, d_in, d_mask, m, n, 25, 25);
		gpu.ReadFromBuffer(results.data(), d_out, m * n *sizeof(float));
...
TEST_CASE( "Test that MAD of each Row is correctly calculated..", "[compute_mad_of_rows], [rfi]" ) {
	...
	for (size_t test = 0; test < 10; test++) {
		...
		// Run on GPU.
		gpu.MADRows(d_out, d_in, m, n, local_size);
